...
/**
 * The persistent class for the expenses_acct database table.
 * 
 */
@Entity
@Table(name="expenses_acct")
@NamedQuery(name="ExpensesAcct.findAll", query="SELECT e FROM ExpensesAcct e")
public class ExpensesAcct implements Serializable {
	...
	public ExpensesAcct(ATransaction atrans, ATransaction aTransaction,String categoryType,String detailType, ExpenseTransaction expenseTransaction) {
	...
	public ExpensesAcct(String categoryType,String detailType, ATransaction aTransaction,ATransaction atrans, ExpenseTransaction expenseTransaction) {
		this.tranDate=aTransaction.getBillDate();
		this.createdDate=new Date();
		this.invoiceNumber=aTransaction.getRefNo();
		this.accountName=atrans.getToAccount();
	...
	public ExpensesAcct(ATransaction aTransaction, String categoryType,String detailType,ExpenseTransaction expenseTransaction) {
		this.tranDate=aTransaction.getBillDate();
		this.createdDate=new Date();
		this.invoiceNumber=aTransaction.getRefNo();
		this.accountName=aTransaction.getToAccount();
		this.payeeName=aTransaction.getVendorName();
		this.transactionType=aTransaction.getTransactionType();
		this.debitAmount=aTransaction.getAmount();
		this.balance=aTransaction.getAmount();
		this.subAccount=detailType;
		this.account=categoryType;
		this.status=aTransaction.getStatus();
		this.client_ID=aTransaction.getClientID();
